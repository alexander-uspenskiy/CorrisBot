...
Когда внутренний агент (Orc, Worker) присылает сообщение → Talker создаёт relay-файл в `tg_corriscant/Prompt_*.md` → тот же looper подхватывает его → Codex LLM пересказывает вместо verbatim relay. Это расходует лишний LLM-вызов и искажает содержимое.
