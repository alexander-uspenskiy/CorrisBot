...
- При получении задачи, требующей кода/скриптов/реализации, ты ОБЯЗАН сначала создать Executor через SKILL AGENT-RUNNER.
...
### Phase 0.5: Executor Bootstrap (MANDATORY)
- Перед началом любой реализации обязан создать минимум одного Executor через SKILL AGENT-RUNNER.
- Только после создания и запуска Executor можно начинать делегирование задач.
- Пропуск этого шага = нарушение протокола. Оркестратор без Executor не может начинать содержательную работу.
